...
function genogramSymbol(data, spouse,twin,idCard) {
        ...
        if(result.length>0) {
            ...
            if (dataFather != null) {
                ...
                rel.CreateTwinRelationship();
                    ...
                    this.GetFamilyDetail(result[0].id, level + 1);
    ...
    this.isOverlap = function(level,x){
        ...
        if(level!==0){
            ...
            if( level == this.hisPosition[i].level && x>=this.hisPosition[i].x && x<=this.hisPosition[i].x+50){
                isOverlap=true;
                break;
            }
            }
...
function TransformData(sourceGenogram){
        ...
        for(var i = 0;i<this.sourceGenogram.length;i++){
            ...
            {   
                ...
                for(var j = 0;j<members.length;j++)
                {
                    ...
                    obj.personId=members[j].properties.id;
                    ...
                    if(obj.father_id!=null && obj.mother_id !=null)
                    {
                        if(obj.father_id!="" && obj.mother_id!="")
                        {
                            ...
                            sp.relationships2 = objPersonInfo2.relationshipsex;
                            this.spouse.push(sp);
                            var sp = {};
                    ...
                    obj.relationships = members[j].properties.relationships;
                    this.data.push(obj);
                    ...
                    if(members[j].properties!=null){
                        if(members[j].properties.relationships!=null){
                            var lstTwin = [];
                            for(var l=0;l <members[j].properties.relationships.length;l++){
                                if(members[j].properties.relationships[l].relate!=null){
                                    if(members[j].properties.relationships[l].relate === 'twin'){
                                        var idCard = this.getIdCardBySysId(members[j].properties.relationships[l].id);
                                        lstTwin.push(idCard);
                                        //lstTwin.push(members[j].properties.relationships[l].id);
                                    }
                                    else if(members[j].properties.relationships[l].relate.toUpperCase() === 'SIBLING'){
                                        //var person = this.GetDataById(members[j].properties.relationships[l].id);
                                        var idCard1 = members[j].idCard;
                                        var idCard2 = this.getIdCardBySysId(members[j].properties.relationships[l].id);
                                        var person1 = this.getPersonInfo(idCard1);
                                        var person2 = this.getPersonInfo(idCard2);
                                  
                                        if(person1.birthday === person2.birthday){
                                          //lstTwin.push(members[j].properties.relationships[l].id);
                                          lstTwin.push(idCard2);
                                        }
                            ...
                            if(lstTwin.length>0){
                                ...
                                strLstTwin = members[j].idCard+','+lstTwin.join(',');
                            }
                            var objTwin ={};
                            if(strLstTwin.trim()!==""){
               ...
               // alert(JSON.stringify(data));
                ...
                if(data.idCard == idCard){
                    ...
                    if(data.properties!=null){
                        ...
                        obj.relationships = data.properties.relationships;
                    }
                    //alert(data.avatarUrl);
                    if(data.properties!=null){

                        var url = data.properties.avatarUrl;
                        if(url!=null){
    ...
    this.getIdCardBySysId = function(id){
        ...
        if(this.sourceGenogram[0].members!=null){
            for(var i = 0 ; i<this.sourceGenogram[0].members.length;i++){
                if(this.sourceGenogram[0].members[i].properties.id==id)
                {
                    idCard = this.sourceGenogram[0].members[i].idCard;
                    break;
                }
            }
        }
    ...
    this.getSysIdByIdCard = function(idCard){
        ...
        if(this.sourceGenogram[0].members!=null){
            for(var i = 0 ; i<this.sourceGenogram[0].members.length;i++){
                if(this.sourceGenogram[0].members[i].idCard==idCard)
                {
                    id = this.sourceGenogram[0].members[i].properties.id;
                    break;
                }
            }
        }
    ...
    this.getRelationStatus = function(id1,id2){
        ...
        if(this.sourceGenogram[0]!=null)
        {
            var source = this.sourceGenogram[0];
            for(var i = 0 ; i<source.members.length;i++){
                if(source.members[i].idCard==id1)
                {
                    for(var j = 0;j<source.members[i].properties.relationships.length;j++){
                        if(source.members[i].properties.relationships[j].id==id){
